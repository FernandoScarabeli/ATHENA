import { ArrowLeft, Check, ChevronDown, History, Info, MessageCircle, MoreHorizontal, Save } from 'lucide-react';
...
type Props = {
  ...
  commentCount: number; panel: DocumentPanel; archiveDisabled: boolean; historicalPreview?: boolean;
  onTitle: (title: string) => void; onClose: () => void; onSave: () => void; onArchive: () => void; onPanel: (panel: DocumentPanel) => void;
};

export function DocumentHeader({ code, revision, title, editable, dirty, saving, error, commentCount, panel, archiveDisabled, historicalPreview = false, onTitle, onClose, onSave, onArchive, onPanel }: Props) {
