...
function SyncRunAccordion({ workspaceId, run, expanded, onToggle }: { workspaceId: string; run: GoogleSyncRun; expanded: boolean; onToggle: () => void }) {
  ...
  return <article className={`sync-run ${expanded ? 'is-open' : ''}`}>
    <button className="sync-run-trigger" aria-expanded={expanded} aria-controls={panelId} onClick={onToggle}>
      <span className="sync-run-folder"><FolderSync size={19}/></span><span className="sync-run-title"><strong>{run.folder.name}</strong><small>{run.folder.project ? `${run.folder.project.key} · ${run.folder.project.name}` : 'Projeto não definido'}</small></span><span className={`sync-run-status status-${run.status.toLowerCase()}`}>{runLabels[run.status]}</span><span className="sync-run-summary"><span className="sync-run-change-count"><b>{run.changedCount}</b> {run.changedCount === 1 ? 'alteração' : 'alterações'}</span><small>{formatDate(run.startedAt)}</small></span><ChevronDown className="sync-run-chevron" size={18} aria-hidden="true"/>
    </button>
    {expanded && <div className="sync-run-panel" id={panelId}>
      ...
      {!page.isLoading && !page.isError && !items.length && run.status !== 'FAILED' && <div className="sync-run-empty">Nenhuma alteração foi encontrada nesta execução.</div>}
...
export function IntegrationCandidatesPanel({ workspaceId, projectId, role }: { workspaceId: string; projectId: string; role?: WorkspaceRole }) {
  ...
  return <section className="list-page integrations-page"><header className="integrations-heading"><div><h1>Integrações</h1><p>Conecte o Google Drive para sincronizar documentos com o projeto.</p></div></header><GoogleDrivePanel workspaceId={workspaceId} projectId={projectId} role={role}/>
    <section className="integration-activity" aria-labelledby="integration-activity-title"><header><div><h2 id="integration-activity-title">Atividade de sincronização</h2><p>Acompanhe as alterações importadas e as sincronizações que precisam de atenção.</p></div><span className="count-summary">{records.length} {records.length === 1 ? 'registro' : 'registros'}</span></header>
      ...
      {!runs.isLoading && !runs.isError && !records.length && <div className="integration-activity-empty" role="status"><span><FolderSync size={21}/></span><div><strong>Sem atividade por enquanto</strong><p>Aqui aparecem alterações importadas e falhas. Sincronizações sem mudanças ficam ocultas.</p></div></div>}
